...
uint8_t ssd1306_WriteChar(uint8_t ch, FontDef Font, SSD1306_COLOR color, uint8_t lcd_id)
{
	...
	{
		b = Font.data[(ch - FONT_TABLE_FIRST_SYMBOL) * Font.FontHeight + i];
...
//
uint8_t ssd1306_WriteString(uint8_t x, uint8_t y, char* str, FontDef Font, SSD1306_COLOR color,uint8_t lcd_id)
{
	ssd1306_SetCursor(x,y, lcd_id);
...
void UpdateGridBox(char* str,uint8_t lcd_id)
{
	...
	if (grid_y>3) {
		for (i=0;i<3;i++) {
			...
			ssd1306_WriteString(0,i*11+16,str_temp,Font_7x10,White,lcd_id);
		}
		y=0;
		...
		ssd1306_WriteString(0,grid_y*11+16, str_temp,Font_7x10,White,lcd_id);
		ssd1306_UpdateScreen(lcd_id);
		grid_y++;
	}
}
...
void ShowClapsTitle(uint8_t lcd_id)
{
	...
	ssd1306_WriteString(0,0,"CLAPS !!!",Font_16x26,White,lcd_id);
	ssd1306_UpdateScreen(lcd_id);
}
